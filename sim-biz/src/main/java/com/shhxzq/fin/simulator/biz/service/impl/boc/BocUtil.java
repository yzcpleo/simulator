...
public class BocUtil {
    ...
    public static BankCommand buildTransaction(Request request, BankTran bankTran, String serNo, String code) {
        BankCommand transaction = new BankCommand();
        transaction.setMerSerialNo(request.getBody().getTraceNo());// 基金公司流水号
        transaction.setBnkSerialNo(serNo);// 银联方流水号
        transaction.setBnkCo(bankTran.getBnkCo());// 银行编号
        transaction.setBnkNm(bankTran.getBnkNm());
        transaction.setRcvrAccoNo(request.getBody().getCardNo());// 交易账户(卡号)
