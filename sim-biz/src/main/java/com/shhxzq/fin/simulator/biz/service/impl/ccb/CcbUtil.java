...
public class CcbUtil {
    ...
    public static BankCommand buildTransaction(Tran tran, BankTran bankTran, String serNo, String code) {
        BankCommand transaction = new BankCommand();
        transaction.setMerSerialNo(tran.getHeader().getTxseq());// 基金公司流水号
        transaction.setBnkSerialNo(serNo);// 银联方流水号
        transaction.setBnkCo(bankTran.getBnkCo());// 银行编号
        transaction.setBnkNm(bankTran.getBnkNm());
        transaction.setRcvrAccoNo(tran.getBody().getAcctNo());// 交易账户(卡号)
